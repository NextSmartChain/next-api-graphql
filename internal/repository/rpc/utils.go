...
func (next *NextBridge) GasPrice() (hexutil.Big, error) {
	// keep track of the operation
	next.log.Debugf("checking current gas price")

	// call for data
	var price hexutil.Big
	var try uint8
	for {
		err := next.rpc.Call(&price, "next_gasPrice")
		if err != nil {
			next.log.Error("current gas price could not be obtained")
			return price, err
		}
		...
		// all attempts failed
		if try > 2 {
			next.log.Errorf("can not get reasonable gas price from the backend server")
...
func (next *NextBridge) GasEstimate(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	next.log.Debugf("calling for gas amount estimation")

	var val hexutil.Uint64
	err := next.rpc.Call(&val, "next_estimateGas", trx)
	if err != nil {
		// missing required argument? incompatibility between old and new RPC API
		if strings.Contains(err.Error(), "missing value") {
			return next.GasEstimateWithBlock(trx)
		}

		// return error
		next.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
	}
...
func (next *NextBridge) GasEstimateWithBlock(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	next.log.Debugf("calling for gas amount estimation with block details")

	var val hexutil.Uint64
	err := next.rpc.Call(&val, "next_estimateGas", trx, BlockTypeLatest)
	if err != nil {
		// return error
		next.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
